...
suppressMessages({
  ...
  library(scales)
  # Parallelization doesn't work well on Windows; if needed, comment next line
  # and remove parallel and BPPARAM parameters below
...
if (threshold < 1) {
  ...
  if (aposteriori <= 1) {
    cat("\tYou can't perform a DE analysis with a posteriori threshold <= 1.")
    cat(" Please check <DE_contrats.tsv>. Analysis will now stop")
    quit(save = "no", status = 1)
  }
}
...
cat("\tread_counts <--  ", read_counts, ", type: ", data.class(read_counts), "\n",
    sep = "")
cat("\tcondition <--  ", condition, ", type: ", data.class(condition), "\n",
    sep = "")
cat("\ttarget <--  ", target, ", type: ", data.class(target), "\n",
    sep = "")
cat("\tcontrol <--  ", control, ", type: ", data.class(control), "\n",
    sep = "")
cat("\tthreshold <--  ", threshold, ", type: ", data.class(threshold), "\n",
    sep = "")
cat("\tfit_type <--  ", fit_type, ", type: ", data.class(fit_type), "\n",
    sep = "")
cat("\tfilter <--  ", filter, ", type: ", data.class(filter), "\n",
    sep = "")
cat("\talpha <--  ", alpha, ", type: ", data.class(alpha), "\n",
    sep = "")
cat("\tdeg_table <--  ", deg_table, ", type: ", data.class(deg_table), "\n",
    sep = "")
cat("\tplots_pdf <--  ", plots_pdf, ", type: ", data.class(plots_pdf), "\n",
    sep = "")


cat("Processing metadata\n")
cat("\tImporting metadata from\n")
metadata <- data.table(Sample = c("lowCO2_sample1", "lowCO2_sample2",
                                  "lowCO2_sample3", "highCO2_sample1",
                                  "highCO2_sample2", "highCO2_sample3"),
                       Condition_type = c("Treatment", "Treatment",
                                          "Treatment", "Treatment",
                                          "Treatment", "Treatment"),
                       Condition_value = c("lowCO2", "lowCO2",
                                           "lowCO2", "highCO2",
                                           "highCO2", "highCO2"))
...
cts <- fread(input = read_counts, header = TRUE, sep = "\t", quote = FALSE,
             stringsAsFactors = FALSE, verbose = FALSE, showProgress = TRUE,
...
if (all(metadata[, Sample] == colnames(cts)[-1])) {
  ...
  cat("\tColumns of count table are different from rows of samples information")
  cat(" table. This will cause DESeq2 to crash. Please check log and sample")
  cat(" names in count table. Analysis will now stop\n")
  quit(save = "no", status = 1)
}
...
dds <- DESeq(object = dds, test = "Wald", sfType = "poscounts",
             betaPrior = FALSE, fitType = fit_type,
             modelMatrixType = "standard", parallel = TRUE,
...
suppressWarnings(
  tmp <- pheatmap(mat = sample_dist_mat,
                  ...
                  treeheight_col = 75, legend = TRUE, show_rownames = TRUE,
                  show_colnames = TRUE, fontsize = 13, fontsize_row = 10,
                  fontsize_col = 10, angle_col = 315, display_numbers = TRUE,
                  number_format = "%.1f", number_color = "red",
                  na_col = "grey", fontsize_number = 10, silent = TRUE)
)
plot_list <- c(plot_list, list(tmp))
rm(tmp)
...
# This commands gives mean of normalised counts across samples for top 20 genes
# with most reads on average
...
suppressWarnings(
  tmp <- pheatmap(assay(blind_vst)[select, ], legend = TRUE,
                  show_rownames = FALSE, cluster_rows = FALSE,
                  cluster_cols = FALSE, border_color = "grey50",
                  na_col = "grey", annotation_col = annot_col, silent = TRUE,
                  show_colnames = TRUE, fontsize = 13, fontsize_row = 9,
                  fontsize_col = 9, angle_col = 315,
                  main = bquote("log2 counts of top 20 genes in S. cerevisiae"))
)
plot_list <- c(plot_list, list(tmp))
rm(tmp)
...
# Cook's distances to check if a sample has a more outliers than others
...
res <- results(object = dds, name = contrast, lfcThreshold = log2(threshold),
               ...
               cooksCutoff = TRUE, parallel = TRUE,
...
if (threshold == 1) {
  is_de <- ifelse(is.na(res[["padj"]]), FALSE,
                  res[["padj"]] < alpha &
  ...
  res_ma <- DESeq2::plotMA(res, alpha = alpha, returnData = TRUE)
  names(res_ma)[names(res_ma) == "isDE"] <- "is_de"
  res_ma_shrunken <- DESeq2::plotMA(res_lfc, alpha = alpha, returnData = TRUE)
  names(res_ma_shrunken)[names(res_ma_shrunken) == "isDE"] <- "is_de"
}

cat("\tPlotting MA values\n")
tmp <- ggplot(data = res_ma, aes(x = mean, y = lfc, color = is_de)) +
  geom_point(size = 1) +
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
...
tmp <- ggplot(data = res_ma_shrunken, aes(x = mean, y = lfc, color = is_de)) +
  geom_point(size = 1) +
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  scale_y_continuous(limits = c(-3, 3), oob = squish) +
...
deg_final <- final[abs(shrunken_log2FoldChange) >= ifelse(threshold > 1,
                                                          ...
                                                          log2(aposteriori))
                   & padj < alpha, ]
fwrite(x = deg_final, file = deg_table, quote = FALSE, sep = "\t",
